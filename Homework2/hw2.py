...
def remove_range(alist, min, max):
   ...
   *** Important: your code must use comprehensions and should not be more than
   two lines of code including the return statement ***
    """
    return [x for x in alist if not min <= x <= max]
...
def friend_list(friend_dictionary):
    ...
    result = {} 
    for person1, person2 in friend_dictionary.items():
        if person1 not in result:
            result[person1] = []
        result[person1].append(person2)
        if person2 not in result:
            result[person2] = []
        result[person2].append(person1)
    return result   
    pass
